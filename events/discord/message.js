...
module.exports = async (bot, message) => {
  ...
  const settings = message.settings = bot.getSettings(message.guild);
  ...
  if (!message.guild) await bot.logger.log(`Recieved DM from ${message.author.tag}: ${message.content}`);
  ...
  !bot.discord.recentChatters.has(message.author.id)) {
    // Add user to recent chatters, then remove them after 2.5 seconds.
    bot.discord.recentChatters.add(message.author.id);
    setTimeout(() => {
      bot.discord.recentChatters.delete(message.author.id);
    }, 2500);
    const key = `${message.guild.id}-${message.author.id}`;
    bot.discord.userProfiles.ensure(key, {
    ...
    bot.discord.userProfiles.math(key, "+", xpAmount, "xp");
    
    // Calculate the user's current level, and adjust if needed.
    const currentLevel = Math.floor(0.1 * Math.sqrt(bot.discord.userProfiles.get(key, "xp")));
    if (currentLevel > bot.discord.userProfiles.get(key, "level")) {
      const nickname = (message.member.nickname) ? message.member.nickname : message.author.username;
      bot.discord.userProfiles.inc(key, "level");
  ...
  const level = bot.discord.permlevel(message);

  // Check whether the command, or alias, exist in the collections defined
  // in app.js.
  const cmd = bot.discord.commands.get(command) || bot.discord.commands.get(bot.discord.aliases.get(command));
  ...
  if (level < bot.discord.levelCache[cmd.conf.permLevel]) {
    if (settings.systemNotice === "true") {
      return message.channel.send(`You do not have permission to use this command.
  Your permission level is ${level} (${bot.config.permLevels.find(l => l.level === level).name})
  This command requires level ${bot.discord.levelCache[cmd.conf.permLevel]} (${cmd.conf.permLevel})`);
    } else {
      return;
    }
  }
  ...
  cmd.run(discordClient, message, args, level, bot);
};
